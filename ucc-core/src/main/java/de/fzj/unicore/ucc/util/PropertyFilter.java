...
public class PropertyFilter implements Filter{
	...
	public boolean filterMatches(BaseWSRFClient wsrf) {
		try{
			...
			//get property and skip to its text content
			XmlCursor c=XmlObject.Factory.parse(wsrfProp).newCursor();
			do{
				while(c.toNextToken().intValue()!=TokenType.START.intValue());
			}while(!property.equalsIgnoreCase(c.getName().getLocalPart()));
			
			while(c.toNextToken().intValue()!=TokenType.TEXT.intValue());
			String propValue=c.getChars();
			c.dispose();
			msg.verbose("Property value: "+propValue);
